<?php 
// Controlador de Roles: gestiona CRUD de roles de usuario
	class Roles extends Controllers{
		 // Constructor: inicia sesión y verifica permisos de acceso
		public function __construct()
		{
			...
			getPermisos(2);// ID del módulo de roles
		}
		// Muestra la vista principal del módulo de roles
		...
		// Devuelve la lista de roles con botones de acciones (JSON para DataTables)
		public function getRoles()
		{
			if($_SESSION['permisosMod']['r']){
				...
				for ($i=0; $i < count($arrData); $i++) {
					// Mostrar estado en formato visual (badge)
					...
					// Botones según permisos del usuario logueado
		...
		// Devuelve un rol específico (por ID) en formato JSON
		...
		public function setRol(){
				...
				// Validar y responder según resultado
				if($request_rol > 0 )
				{
					if($option == 1)
					{
		...
		// Elimina un rol por ID (si no está asignado a usuarios)
