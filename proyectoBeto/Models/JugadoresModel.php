...
class JugadoresModel extends Mysql
{
	...
	public function insertJugador(string $club,string $codigo, string $nombre, string $apellido, string $carnet, int $status, string $fechaJuego)
	{
		$this->club = $club;
		$this->codigo = $codigo;
		$this->nombre = $nombre;
	...
	public function updateJugador(int $id, int $club, string $nombre, string $apellido, string $carnet, int $status, string $fechaJuego)
	{
		$this->id = $id;
		//$this->codigo = $codigo;
		$this->club = $club;
		...
		if (empty($request)) {
			$sql = "UPDATE jugadores SET club_id=?, nombre=?, apellido=?, carnet=?, status=?, fechajuego=? 
					WHERE id = ?";
			$arrData = array(
				$this->club,
				//$this->codigo,
